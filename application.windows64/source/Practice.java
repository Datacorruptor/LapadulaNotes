...
public void keyPressed()//метод вызываемый при нажатии на клавиатуру
{
...
public void checkfornewline(int cont)// проверка для переноса на новую строку
{
...
public boolean verifyLogin(String l, String p)// метод проверки логина
{
...
public boolean verifyReg(String l, String p, String c)//метод проверки регистрации
{
  for (member v : members)//смотрим всех пользователей
  {
...
public void createNote()//создание заметки
{
  int l = PApplet.parseInt(container[1]);//берём число из контейнера уровня (если не число, то равно 0)
...
public void deleteNote()//удаление заметки
{
...
public void viewNote()//просмотр заметки
{
...
public void writeNote()//запись заметки
{
...
public void logout()//метод выхода пользователя
{
  for (int i=0; i<container.length; i++)//очищаем все контейнеры
    container[i]="";
